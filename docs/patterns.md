...
| `denseScatteredRings` | `flowerRings` の高密度版。同じ位置の決め方・速さ（1.8）で、48 発の全方位リングを 18 フレーム間隔で撃つ（`flowerRings` の約 1.8 倍の密度）。隙間が狭くなりすぎないよう弾の半径を 4 から 3 に小さくしている | `denseScatteredRings`（新規追加） | （未定） |
...
| `twinScatteredRings` | `flowerRings` のリング（36 発、速さ 1.8、24 フレーム間隔）を、ボスの左側と右側の 2 箇所から同時に撃つ。位置は左右それぞれの範囲（ボスから横に 40〜150）でランダムに選ぶので重ならない。2 つのリングが交差して隙間の形が複雑になる。左右で色を変えている | `twinScatteredRings`（新規追加） | （未定） |
| `centerRandomPick` | ボスの揺れを止めて画面中央（縦横とも）へ移動し、固定の候補（`spiral` / `aimedFan` / `flowerRings` / `doubleScatteredRings` / `denseScatteredRings` / `delayedAimScatteredRings` / `burstingRings` / `sideSpiralEnemies` / `evenFanStreamWithAimedShot`）から 1 つ抽選して撃つ。撃ち終わったら元の y 座標へ戻り、揺れを再開する | `centerRandomPick`（新規追加） | （未定） |
...
| `normal` | `doubleScatteredRings` / `denseScatteredRings` / `delayedAimScatteredRings` / `burstingRings` / `sideSpiralEnemies` / `centerRandomPick` |
| `hard` | `evenFanStreamWithAimedShot` / `spiralRain` / `wallBounceRings` / `twinScatteredRings` / `evenFanRandomMix` |
