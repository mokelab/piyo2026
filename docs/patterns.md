...
| `centerRandomPick` | ボスの揺れを止めて画面中央（縦横とも）へ移動し、固定の候補（`spiral` / `aimedFan` / `flowerRings` / `doubleScatteredRings` / `delayedAimScatteredRings` / `sideSpiralEnemies`）から 1 つ抽選して撃つ。撃ち終わったら元の y 座標へ戻り、揺れを再開する | `centerRandomPick`（新規追加） | （未定） |
